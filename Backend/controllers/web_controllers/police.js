...
export const getPoliceStationAnalytics = async (req, res) => {
    try {
        ...
        if (!policeStation) return res.status(404).json({ error: "Police station not found" });
        ...
        const alertsLast7Days = alerts.filter((alert) => alert.Time >= sevenDaysAgo);
        ...
        const crimeCounts = Object.entries(types).map(([name, count]) => ({ name, count }));
        ...
        const response = {
            ...
            types: Object.entries(types).map(([name, count]) => ({
                name,
                percentage: parseFloat(((count / alertsCurrentMonth.length) * 100).toFixed(2)), // Rounded to 2 decimal places
            })),
            crimeCounts,
            monthly,
            daily,
